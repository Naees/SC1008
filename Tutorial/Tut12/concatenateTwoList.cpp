...
Write a C++ function to concatenate two separate linked lists. The function prototype is
as follows:
void concateTwoLists (Node*& firstList, Node*& secondList);
The function should append the second linked list to the end of the first linked list.
After the concatenation, secondList should be set as nullptr.*/
...
void concateTwoLists(Node*& firstList, Node*& secondList) {
    ...
    // If the first list is empty, the concatenated list is just the second list.
    ...
    // Set the secondList to nullptr to indicate it is now empty
    secondList = nullptr;
}
